...
- (void) setUpLayers {
    ...
    gestureRight = [[[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeRight:)] autorelease];//direction is set by default.
    gestureLeft = [[[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeLeft:)] autorelease];//need to set direction.
...
//    pinchGestureRecognizer=[[[UIPinchGestureRecognizer alloc]initWithTarget:self action:@selector(pinched:)]autorelease];
...
//    for (int i=0; i<5; i++) {
//        if (i<numberOfPages) {
//            [pageCache precacheImageForPageIndex:i];
//
//        }
//    }
}
...
- (void)swipeRight:(UISwipeGestureRecognizer *)gesture
{
    if ((gesture.state == UIGestureRecognizerStateChanged) ||
        ...
        [self.layer addAnimation:animation forKey:@"pageSwiped"];    
        self.currentPageIndex-=1;
        [self willTurnToPageAtIndex:self.currentPageIndex];
        //self.leafEdge = 1.0;
        interactionLocked = YES;
        if (self.currentPageIndex-1>numberOfPages) {
                [pageCache precacheImageForPageIndex:self.currentPageIndex-1];
        }
